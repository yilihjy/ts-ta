...
describe("Test EMA", () => {
    ...
    it("test EMA()", () => {
        ...
        for (let i = 1; i < data.length; i++) {
            expect(data[i]["EMA12"]).toBeCloseTo(
                (+data[i].close - data[i - 1][`EMA12`]) * (2 / (12 + 1)) + data[i - 1][`EMA12`])
            expect(data[i]["EMA50"]).toBeCloseTo(
                (+data[i].close - data[i - 1][`EMA50`]) * (2 / (50 + 1)) + data[i - 1][`EMA50`])
        }
    });
    ...
    it("test EMA(15,60)", () => {
        ...
        for (let i = 1; i < data.length; i++) {
            expect(data[i]["EMA15"]).toBeCloseTo(
                (+data[i].close - data[i - 1][`EMA15`]) * (2 / (15 + 1)) + data[i - 1][`EMA15`])
            expect(data[i]["EMA60"]).toBeCloseTo(
                (+data[i].close - data[i - 1][`EMA60`]) * (2 / (60 + 1)) + data[i - 1][`EMA60`])
        }
    });
